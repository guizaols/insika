...
module Harness
  ...
  # per project (NF1): the engine doesn't know achei-b2b — the pack is the contract.
  #
  ...
  Pack = Data.define(:config, :files, :skills, :tools) do
    ...
    def self.from_dir(path)
      ...
      raise Harness::ValidationError, "pack missing agent.config.json in #{root}" unless File.exist?(config_file)
