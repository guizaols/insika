...
module Harness
  ...
  #     sidecar's LITESTREAM_*, the deployment's DEEPSEEK_*/CONSUMER_*) are never flagged.
  ...
  #     or `strict: true`).
  #
  # Harness::Doctor reuses `validate` for its `env:*` checks; the boot roots call
  ...
  module EnvSchema
    ...
    def enforce!(env = ENV, extra: [], strict: nil, warn: method(:default_warn))
      ...
      raise Harness::ConfigError.new("strict config check refused boot", findings: errors) if strict && errors.any?
