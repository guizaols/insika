...
module Harness
  # Registry de tools (doc 06 §2). Herda o genérico; `optional`/`side_effect`
  # vivem no metadata (defaults false). `side_effect: true` marca tool
  # não-idempotente — o mecanismo de checkpoint (doc 02 §3, RFC-0006 §5)
  # consome via `side_effect?`.
  class ToolRegistry < Registry
    ...
    # Compat Fase 0: `optional:`/`plugin:` continuam aceitos; `side_effect:` é
    ...
    # -> bool; consumido pelo ToolEnvelope (doc 02 §2). Tool desconhecida = false.
    def side_effect?(name)
      entry = entries.find { |e| e.name == name.to_s }
      entry ? !!entry.metadata[:side_effect] : false
    end

    # Despacho por tipo: AgentProfile -> atalho DEPRECATED (doc 05 §8, doc 06 §8),
    # delega ao Policy::Engine com a ToolAllowlist e devolve INSTÂNCIAS (retorno
    # compatível com a Fase 0). Caso contrário -> lookup por nome (genérico).
    def resolve(arg)
