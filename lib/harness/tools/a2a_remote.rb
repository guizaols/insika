...
module Harness
  module Tools
    ...
    # allowlist do agente governa quem pode delegar. require lazy da gem (D9): NÃO
    # entra em lib/harness.rb; é carregado no bloco de registro (wiring) na 1ª
    # instância. Uma instância por agente remoto (name/description próprios, L1).
    class A2ARemote < RubyLLM::Tool
      ...
      # senão o modelo veria "harness--tools--a2a_remote" p/ todos os remotos, L1).
      def name = @tool_name
      ...
      # do wire_callbacks já correlaciona a chamada em si. (L3)
      def emit(result)
