...
module Harness
  ...
  class WorkflowRegistry < Registry
    ...
    def definition(name)
      e = entry(name)
      Harness::Workflow::Definition.new(
        name: e.name,
        description: e.metadata[:description],
        input_schema: Harness::Workflow::Schema.coerce(e.metadata[:input_schema]),
        output_schema: Harness::Workflow::Schema.coerce(e.metadata[:output_schema]),
        factory: e.factory
      )
    end
