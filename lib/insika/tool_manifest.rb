  ...
  class ToolManifest
    ...
    def resolve_url(t, env)
      ...
      base = self.class.presence(defaults["base_url"]) ||
             (raise Insika::ValidationError, "tool '#{t['name']}' has no url and manifest has no defaults.base_url")
      path = self.class.presence(defaults["path_template"]) ||
             (raise Insika::ValidationError, "tool '#{t['name']}' has no url and manifest has no defaults.path_template")
      endpoint = self.class.presence(t["endpoint"]) ||
                 (raise Insika::ValidationError, "tool '#{t['name']}' has no 'endpoint' (never inferred from the name — R6)")
    ...
    def fetch!(resolver, key, kind)
      val = resolver[key]
      raise Insika::ValidationError, "#{kind} '#{key}' not configured in the deployment" if blank?(val)
    ...
    def guard_no_stray_secrets!(url:, headers:, secret_headers:, query:, body:)
      ...
      stray = strings.flat_map { |s| s.to_s.scan(Insika::ToolDefinition::PLACEHOLDER_RE).flatten }
