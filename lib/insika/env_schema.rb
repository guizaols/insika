...
module Insika
  ...
  module EnvSchema
    ...
    DEFAULT = [
      ...
      spec(name: "INSIKA_ROUTER_BACKENDS", type: :csv, description: "RFC-0043: comma-separated backend URLs for `insika-router` (static discovery, the Railway shape). Exactly one of this or INSIKA_ROUTER_BACKENDS_DNS."),
      spec(name: "INSIKA_ROUTER_BACKENDS_DNS", description: "RFC-0043: a headless-Service hostname `insika-router` re-resolves on an interval (the Kubernetes shape). Requires INSIKA_ROUTER_BACKEND_PORT."),
      spec(name: "INSIKA_ROUTER_BACKEND_PORT", type: :integer, description: "RFC-0043: the engine port on every DNS-resolved backend pod (required with INSIKA_ROUTER_BACKENDS_DNS)."),
      spec(name: "INSIKA_ROUTER_DNS_INTERVAL", type: :integer, description: "RFC-0043: seconds between `insika-router` DNS re-resolves (default 15)."),
      spec(name: "INSIKA_ROUTER_BODY_MAX_BYTES", type: :integer, description: "RFC-0043: size cap `insika-router` will parse looking for a session key before falling back to round-robin (default 262144; never bounds what is forwarded)."),
      spec(name: "INSIKA_ROUTER_BACKEND_TIMEOUT", type: :integer, description: "RFC-0043: `insika-router`'s connect/read timeout to a backend, in seconds (default 10)."),
      spec(name: "INSIKA_ROUTER_HOST", description: "RFC-0043: bind address for `insika-router` itself (default 0.0.0.0)."),
      spec(name: "INSIKA_ROUTER_PORT", type: :integer, description: "RFC-0043: listen port for `insika-router` itself (default 9090)."),
