...
module Insika
  ...
  # `continuation`: optional partial-turn messages and metadata for approval resume.
  Checkpoint = Data.define(:task_id, :turn, :session_id, :agent_id,
                           :messages, :completed_side_effects, :created_at, :continuation) do
    def initialize(continuation: nil, **attributes)
      super(**attributes, continuation: continuation)
    end
  end
end
