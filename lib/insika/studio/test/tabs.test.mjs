...
test("no hash but a server-declared default opens that tab (frame nav dropped the fragment)", () => {
  globalThis.location.hash = ""
  const a = mk("config"), b = mk("prompts")
  const c = build([a, b], [a, b])
  c.defaultValue = "prompts"
  c.connect()
  assert.equal(a.hidden, true)
  assert.equal(b.hidden, false)
  c.disconnect()
...
test("an actual hash still wins over the server-declared default", () => {
  globalThis.location.hash = "#config"
  const a = mk("config"), b = mk("prompts")
  const c = build([a, b], [a, b])
  c.defaultValue = "prompts"
