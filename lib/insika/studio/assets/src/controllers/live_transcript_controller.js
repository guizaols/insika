...
export default class extends Controller {
  ...
  connect() {
    ...
    this.currentTag = null    // its .msg-role label node
  ...
  // One-line args preview for a collapsed tool-call chip; body keeps the full JSON.
  // Bare text — the chip's .chip-sub styling carries the affordance.
  argsPreview(args) {
  ...
  // One word for the collapsed summary: the shared reason when every body arrived
  // the same way, "mixed" when they did not. A turn CAN mix — the agent's eager set
  // plus whatever this message triggered — which is why a single per-turn mode was a
  // lie waiting to happen.
  skillMode(skills) {
    const kinds = [...new Set(skills.map((s) => String(s.reason || "").split(":")[0]))]
    if (kinds.length === 0 || kinds.includes("")) return "context"
    return kinds.length === 1 ? kinds[0] : "mixed"
  }

  // Clones a chip icon out of the #chip-icons <template> the layout renders
  // (one source of truth: nav_icons.rb — the ERB partial's chip_icon uses the
  // same sprite). null when the sprite is absent (tests, exotic embeds) —
  // callers treat an iconless chip as fine.
  chipIcon(kind) {
    const sprite = document.getElementById("chip-icons")
    const node = sprite && sprite.content.querySelector(`[data-kind="${kind}"]`)
    return node ? node.cloneNode(true) : null
  }

  // Collapsible event chip: <details> with a pill summary (icon + title +
  // optional one-line args preview + optional ok/err badge) and the raw
  // payload in a <pre> body. Mirrors _message.erb — change both or neither.
  toolcard(kind, title, sub, body, badge) {
    const c = this.el("details", "toolcard " + kind)
    const s = document.createElement("summary")
    const ic = this.chipIcon(kind)
    if (ic) s.appendChild(ic)
    s.appendChild(this.el("strong", null, title))
    if (sub) s.appendChild(this.el("span", "chip-sub", sub))
  ...
  // A visible, always-open link for a save_artifact result — the toolcard
  // below is <details> (collapsed by default; the raw {id,url} payload is
  // useful for debugging, not for finding the report). Mirrors _message.erb's
  // artifact_link_in for the stored-history render of the SAME turn after a
  // reload — both need to exist: this is the live SSE path, that one replays
  // session.messages, and they see the result in different shapes (a real
  // object here; a Hash#to_s string there).
  artifactChip(url) {
    const p = this.el("p", "artifact-chip")
    const ic = this.chipIcon("artifact")
  ...
  // Markup mirrors _message.erb: a mono label row above a tail-less
  // bubble — no avatar, the label row IS the voice.
  appendDelta(ev, draft) {
    if (!this.current) {
      const msg = this.el("div", "msg assistant")
      this.currentMeta = this.el("div", "msg-meta")
      this.currentTag = this.el("span", "msg-role")
      this.currentMeta.appendChild(this.currentTag)
      const stamp = this.time(ev.meta && ev.meta.at)
      if (stamp) this.currentMeta.appendChild(stamp)
      msg.appendChild(this.currentMeta)
  ...
  render(ev) {
    ...
    switch (ev.type) {
      ...
      case "tool_call":
        this.finishBubble()
        this.push(this.toolcard("call", ev.name || "tool", this.argsPreview(ev.arguments), ev.arguments))
        break
      case "tool_result": {
        const ok = !(ev.result && typeof ev.result === "object" && "error" in ev.result)
        if (ok && ev.name === "save_artifact" && ev.result && typeof ev.result.url === "string") {
          this.push(this.artifactChip(ev.result.url))
        }
        this.push(this.toolcard("result", ev.name || "tool", null, ev.result,
      ...
      // path carries `skills` — {name, reason} each, a provider injected the bodies
      // and no call happened — so rendering it as `load_skill(...)` would invent a
      // tool call. The REASON per skill is the ask this card exists for: "which one
      // did I trigger" is unanswerable from a bare list of names.
      case "skill_activated":
        this.finishBubble()
        if (Array.isArray(ev.skills)) {
          this.push(this.toolcard("skill",
            `skills · ${this.skillMode(ev.skills)} (${ev.skills.length})`, null,
            ev.skills.map((s) => (s.reason ? `${s.name} · ${s.reason}` : s.name)).join("\n")))
        } else {
      ...
      case "guardrail_blocked":
        ...
        // follows via :content). Distinct chip — audit at a glance.
        this.finishBubble()
        this.push(this.toolcard("guardrail", "guardrail blocked · " + (ev.category || "?"), null,
          [ev.source, ev.action, ev.detail].filter(Boolean).join(" · ") || null))
        break
      case "guardrail_flagged":
        this.finishBubble()
        this.push(this.toolcard("guardrail", "guardrail flagged · " + (ev.category || "?"), null,
