...
export default class extends Controller {
  ...
  selectSection(event) {
    const section = event.currentTarget.closest(".tool-section")
    const checked = event.currentTarget.dataset.checked === "true"
    if (this.hasAllTarget) this.allTarget.checked = false
    this.toolTargets.forEach((tool) => {
      if (section.contains(tool) && tool.dataset.locked !== "true") tool.checked = checked
    })
    this.update()
  }
  ...
  update() {
    ...
    const on = this.toolTargets.filter((t) => t.checked && t.dataset.locked !== "true").length
