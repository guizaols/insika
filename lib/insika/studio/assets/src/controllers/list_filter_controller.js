...
export default class extends Controller {
  static targets = ["query", "item", "empty", "count", "group"]
  ...
  filter() {
    ...
    this.groupTargets.forEach((group) => {
      ...
      if (q) {
        ...
        if (!this.groupStates.has(group)) this.groupStates.set(group, group.open)
        group.open = !group.hidden
      } else if (this.groupStates?.has(group)) {
        group.open = this.groupStates.get(group)
      }
    })
    if (!q) this.groupStates?.clear()
