# Harness — build your first agent

> **You are a coding agent** (Claude Code, Cursor, an IDE assistant, …) reading this
> on behalf of a developer who just pointed you at a running Harness. Treat this file
> as a **skill**: follow the steps in order, apply the RULES literally, and stop at the
> self-check. Do **not** improvise beyond it.

Harness is a Ruby runtime for LLM agents in production. Your job here is the smallest
possible one: get the developer a **first working agent**, defined in Ruby with the
public DSL (`Harness.agent { … }`), talking to a real model — nothing more.

The machine-readable list of models this harness already knows about is at
...
- **The gem/library must be loadable.** In a project that already depends on Harness,
  `require "harness"` works. Otherwise add it to the `Gemfile` (or `bundle add`) — do
...
- **Fetch {{MODELS_URL}}.** It tells you which providers and model ids this harness is
...
| Question | RULE |
|---|---|
| How many agents? | **Exactly one.** A first agent is a single `Harness.agent`. Resist adding a second. |
...
- **Never guess a model id.** Use only ids from {{MODELS_URL}}. A wrong id fails at
  the provider, not in the harness, and wastes the developer's time.
