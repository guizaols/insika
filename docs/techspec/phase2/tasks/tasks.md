...
|---|------|-----------|--------|------------|------|
| 1 | Estender `TaskActor`: enum `%i[cancel user_message approval pause resume timeout heartbeat]` + `drain!` roteando cada um (approval/pause/resume mudam fluxo; timeout/heartbeat observam) | P2-01 | ⬜ TODO | Med | 0002 §9 |
| 2 | Ativar estados no Executor: suspensão em `:paused` (drena `:pause` na fronteira → `transition(:paused)` → aguarda `:resume`) sem perder checkpoint | P2-01 | ⬜ TODO | High | 0002 §9 |
