...
- **Observações / decisões tomadas:**
  ...
  - **Desvio menor do sample (deliberado):** o `audit` e o evento `:policy_denied` usam o **nome declarado** em `profile.policies` (ex.: `"tool_allowlist"`), não `policy.id` (nome da classe) como o Step 3 escreveu — o nome declarado é mais útil p/ audit/Control UI e casa com o perfil. Sem impacto funcional (confirmado no review).
