...
| 7 | `CheckpointStore` (checkpoint por turno, chave avulsa de side-effects, `prune`) | [task-07.md](./task-07.md) | ✅ DONE | Med | 02 §2-§3 |
| 8 | `Recovery` no boot (varredura + dispatch de resume; bus como duplo até a task 13) | [task-08.md](./task-08.md) | ✅ DONE | Med | 02 §4, §6 |
| 9 | `Command` + `CommandBus` + handlers de controle (`CreateSession`, `CancelTask`) | [task-09.md](./task-09.md) | ✅ DONE | Med | 03 §2-§3 |
| 10 | `Executor` esqueleto: fiber por task, `TaskActor` (mailbox `cancel`), estados, `running?`, `EventStream` | [task-10.md](./task-10.md) | ✅ DONE | High | 03 §2, §5 |
...
| 24 | Rotas formais: `POST /v1/commands/:type` + açúcar, reads GET, `/v1/events` SSE (heartbeat, cap), rota legada | [task-24.md](./task-24.md) | ✅ DONE | Med | 07 §2-§4 |
| 25 | Esqueleto Control UI `/admin` read-only (ERB) + `AdminAuth` fail-closed | [task-25.md](./task-25.md) | ✅ DONE | Med | 07 §2, §6 |
| 26 | `Gemfile` pinado + Gemfile.lock + `Server::Boot` (plugins→recovery→listen) + smoke E2E kill -9 | [task-26.md](./task-26.md) | ✅ DONE | Med | 00 D9, 07 §4, §7 |
