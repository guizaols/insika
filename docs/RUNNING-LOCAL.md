...
tools apontando de volta pro achei-b2b. Um **pack** é a pasta:
...
- `request.url` = `http://localhost:3000/api/internal/agent_tools/<rota>`
  ...
  `call_support`→`support_requests`. Confira em `config/routes.rb` do achei-b2b)
...
- o `id` do agente deve bater com o `openclaw_agent_id` da Store no achei-b2b
