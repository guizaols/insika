...
`consumer-tools-dev` do OpenClaw) com o conversor. Ele lê os `.ts` (name, slug do
...
```bash
PLUGIN=/caminho/openclaw/extensions/consumer-tools-dev
...
ruby scripts/openclaw_to_pack.rb "$PLUGIN" ~/Desktop/agent/v2/acme/tools \
  search_products recommend_products add_to_cart update_cart_quantity remove_from_cart \
  reopen_cart reset_cart send_finalize_button search_faq search_orders call_support store_set_location

# 2) re-provisiona (o token interno entra por env; a URL base sai de CONSUMER_INTERNAL_URL)
OPENCLAW_GATEWAY_TOKEN=local-demo BIA_INTERNAL_API_TOKEN=<token> \
bundle exec ruby scripts/import_pack.rb ~/Desktop/agent/v2/acme
```
...
tunnel/simulador — mas isso é do lado do consumer-app.)
