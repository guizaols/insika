...
RSpec.describe Insika::ToolManifest do
  ...
  # typical achei defaults (base_url from env, auth from secret, ctx per turn).
  def achei_defaults
  ...
  describe "resolution of {{env.*}} / {{secret.*}} (task 4)" do
    ...
    it "clear error when env/secret is not configured in the deployment" do
      ...
      expect { defn(m, secrets: secrets, env: {}) }.to raise_error(Insika::ValidationError, /env 'ACHEI_URL'/)
    end
  end
