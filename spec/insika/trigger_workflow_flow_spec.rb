...
RSpec.describe "Executor: trigger_workflow (stage 6 variant)" do
  ...
  let(:policy_registry) do
    {
      ...
      "workflow_allowlist" => Insika::Policy::Builtin::WorkflowAllowlist.new
    }
  end
  ...
  def build_executor(allowed_tools: [])
    Insika::Executor.new(
      ...
      policy_engine: Insika::Policy::Engine.new(policy_registry: policy_registry, event_stream: engine_stream),
      middleware: PassthroughMiddleware.new, hooks: Insika::Hooks.new,
  ...
  def make_task(input: { "q" => "x" }, id: "t")
    command = Insika::Command.build(:trigger_workflow, { workflow: "flow", agent: "sales", input: input })
  ...
  it "WorkflowAllowlist: workflow outside workflows_allow -> :policy_denied + task :failed, workflow never invoked" do
    ...
    profile_no_wf = Insika::AgentProfile.build(id: "sales", model: "gpt",
  ...
  it "omitted input: workflow receives {} (not nil)" do
    ...
    executor = build_executor
    command = Insika::Command.build(:trigger_workflow, { workflow: "flow", agent: "sales" })
  ...
  it "output that violates output_schema -> task :failed at :workflow_schema, no :workflow_completed" do
    ...
    expect(task.executions.last.error["class"]).to eq("Insika::WorkflowSchemaError")
  ...
  it "side-effect + resume: side_effect tool does not re-run on resume (skip by name)" do
    ...
    checkpoint_store.save(Insika::Checkpoint.new(task_id: "t", turn: 1, session_id: nil,
