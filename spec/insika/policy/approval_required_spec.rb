...
RSpec.describe Insika::Policy::Builtin::ApprovalRequired do
  subject(:policy) { described_class.new }

  def request_for(approvals_required)
    profile = Insika::AgentProfile.build(id: "a", model: "m", approvals_required: approvals_required)
    Insika::Policy::PolicyRequest.new(profile: profile, command: nil, context: nil,
  ...
  describe "aggregation in the Engine" do
    let(:registry) do
      reg = Insika::PolicyRegistry.new
      reg.register(:approval_required, described_class)
      reg.register(:tool_allowlist, Insika::Policy::Builtin::ToolAllowlist)
      reg
    end
    let(:engine) { Insika::Policy::Engine.new(policy_registry: registry, event_stream: SpyEventStream.new) }
    ...
    it "Resolution.requires_approval gathers the policy names" do
      ...
      req = Insika::Policy::PolicyRequest.new(
