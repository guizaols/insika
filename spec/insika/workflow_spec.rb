...
RSpec.describe Insika::Workflow do
  describe Insika::Workflow::Schema do
    ...
    it "rejects a non-Hash, non-callable schema at construction" do
      expect { described_class.new("nope") }.to raise_error(Insika::ValidationError)
    end
  end

  describe Insika::Workflow::Definition do
    def definition(**over)
      described_class.new(
    ...
    it "validate_input! raises WorkflowSchemaError(phase: :input) with the field detail" do
      d = definition(input_schema: Insika::Workflow::Schema.new(
        "type" => "object", "properties" => { "q" => { "type" => "string" } }, "required" => ["q"]
      ))
      expect { d.validate_input!({}) }.to raise_error(Insika::WorkflowSchemaError) do |e|
    ...
    it "validate_output! raises WorkflowSchemaError(phase: :output)" do
      d = definition(output_schema: Insika::Workflow::Schema.new(
        "type" => "object", "properties" => { "ok" => { "type" => "boolean" } }, "required" => ["ok"]
      ))
      expect { d.validate_output!({}) }.to raise_error(Insika::WorkflowSchemaError) { |e| expect(e.phase).to eq(:output) }
    end
    ...
    it "works with a duck-typed (dry-schema-compatible) validator and normalizes #errors.to_h" do
      ...
      expect { d.validate_input!({}) }.to raise_error(Insika::WorkflowSchemaError) { |e| expect(e.errors).to eq("q" => ["is missing"]) }
    end
    ...
    it "#catalog_entry exposes name/description and the schema view (Hash vs opaque vs nil)" do
      d = definition(
        ...
        input_schema: Insika::Workflow::Schema.new("type" => "object"),
