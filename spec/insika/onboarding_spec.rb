...
RSpec.describe Insika::Onboarding do
  ...
  describe "#models_json" do
    it "carries the thinking levels and base even with no stores" do
      ...
      expect(json[:thinking_levels]).to eq(Insika::ModelSelection::THINKING_LEVELS)
  ...
  describe "docs" do
    it "indexes the allowlisted docs with a title from the first heading" do
      ...
      expect(index).to eq([{ name: "readme", title: "Insika", url: "http://x/docs/readme.md" }])
    end
