...
require "spec_helper"
require "insika/tools/subagent" # the Executor loads it lazily in create_chat; explicit here

RSpec.describe Insika::Tools::Subagent do
  let(:task) { Struct.new(:id, :session_id).new("t1", "s1") }
  let(:state) do
    profile = Insika::AgentProfile.build(id: "parent", model: "m", subagents: ["researcher"])
    Insika::TurnState.new(task: task, profile: profile, turn: 1, message: "oi")
  end
