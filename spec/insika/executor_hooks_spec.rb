...
RSpec.describe "Insika::Executor + hooks (:task/:agent/:tool)" do
  ...
  it "tool call counter resets per turn (two tasks of 2 calls, limit 3)" do
    profile3 = Insika::AgentProfile.build(id: "sales", model: "gpt", limits: { max_tool_calls: 3 })
    executor = build_executor
  ...
  it "tool_limit: 51st tool call with max_tool_calls 50 -> TimeoutError(stage :tool_limit) -> :failed" do
    fast_profile = Insika::AgentProfile.build(id: "sales", model: "gpt", limits: { max_tool_calls: 2 })
    chat = FakeChat.new
