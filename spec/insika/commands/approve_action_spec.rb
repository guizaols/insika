...
RSpec.describe Insika::Commands::ApproveAction do
  subject(:handler) do
  ...
  def approve(pending_id, decision: "approved", operator: "alice")
    handler.call(Insika::Command.build(:approve_action,
  ...
  it "no-op on the executor without a live fiber (process crashed): store stays resolved" do
    ...
    pa = pending
    h.call(Insika::Command.build(:approve_action, { pending_id: pa.id, decision: "approved" }))
  ...
  it "missing pending_id -> ValidationError" do
    expect { handler.call(Insika::Command.build(:approve_action, { decision: "approved" })) }
      .to raise_error(Insika::ValidationError)
  end

  it "nonexistent pending -> NotFoundError; invalid decision -> ValidationError" do
    expect { approve("ghost") }.to raise_error(Insika::NotFoundError)
    pa = pending
    expect { approve(pa.id, decision: "maybe") }.to raise_error(Insika::ValidationError)
  end

  it "double approval -> ValidationError (only :pending resolves)" do
    pa = pending
    approve(pa.id, decision: "approved")
    expect { approve(pa.id, decision: "rejected") }.to raise_error(Insika::ValidationError)
  end
end
