...
RSpec.describe Insika::Commands::PauseTask do
  ...
  it "posts :pause to the executor and returns the Task" do
    ...
    task = handler.call(Insika::Command.build(:pause_task, { task_id: "t" }))
  ...
  it "is a no-op without a live fiber (executor returns false) — no error" do
    ...
    expect { h.call(Insika::Command.build(:pause_task, { task_id: "t" })) }.not_to raise_error
    expect(dead.paused).to eq(["t"])
  end

  it "raises NotFoundError and does NOT call the executor for a nonexistent task" do
    expect { handler.call(Insika::Command.build(:pause_task, { task_id: "ghost" })) }
  ...
  it "raises ValidationError for a missing/empty task_id" do
    expect { handler.call(Insika::Command.build(:pause_task, {})) }
      .to raise_error(Insika::ValidationError)
    expect { handler.call(Insika::Command.build(:pause_task, { task_id: "" })) }
      .to raise_error(Insika::ValidationError)
  end
