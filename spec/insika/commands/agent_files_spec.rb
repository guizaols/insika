...
RSpec.describe "Agent workspace commands" do
  ...
  before { source.put(Insika::AgentProfile.build(id: "bia", model: "m")) }

  describe Insika::Commands::WriteAgentFile do
    ...
    it "agent_id/file required; nonexistent agent -> NotFoundError" do
      expect { handler.call(cmd(:write_agent_file, { "file" => "x" })) }.to raise_error(Insika::ValidationError, /agent_id/)
      expect { handler.call(cmd(:write_agent_file, { "agent_id" => "bia" })) }.to raise_error(Insika::ValidationError, /file/)
  ...
  describe Insika::Commands::DeleteAgentFile do
    subject(:handler) { described_class.new(profile_source: source, agent_file_store: files, event_stream: stream) }
    ...
    it "removes the file from the agent's prompt_files" do
      source.put(Insika::AgentProfile.build(id: "bia", model: "m", prompt_files: %w[IDENTITY.md SOUL.md]))
  ...
  describe Insika::Commands::RestoreAgentFile do
    subject(:handler) { described_class.new(profile_source: source, agent_file_store: files, event_stream: stream) }
