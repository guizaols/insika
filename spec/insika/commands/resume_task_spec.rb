...
RSpec.describe Insika::Commands::ResumeTask do
  subject(:handler) do
  ...
  def resume(id)
    handler.call(Insika::Command.build(:resume_task, { task_id: id }))
  end
  ...
  it "live running (running? true): ValidationError, does not spawn" do
    seed("t", status: :running)
    executor.running = true
    expect { resume("t") }.to raise_error(Insika::ValidationError, /is running/)
    expect(executor.spawned).to be_empty
  end
  ...
  it "missing task_id: ValidationError" do
    expect { handler.call(Insika::Command.build(:resume_task, {})) }
