...
RSpec.describe Insika::Soak::Runner do
  ...
  describe ".arrivals" do
    it "is deterministic for a fixed seed" do
      expect(described_class.arrivals(seed: 42, turns_per_hour: 60, count: 100))
        .to eq(described_class.arrivals(seed: 42, turns_per_hour: 60, count: 100))
    end

    it "differs across seeds" do
      expect(described_class.arrivals(seed: 1, turns_per_hour: 60, count: 50))
        .not_to eq(described_class.arrivals(seed: 2, turns_per_hour: 60, count: 50))
    end

    it "has a mean inter-arrival within tolerance of 3600 / turns_per_hour over 10k draws" do
      gaps = described_class.arrivals(seed: 7, turns_per_hour: 60, count: 10_000)
  ...
  describe "#run" do
    it "writes header, turns, hourly snapshots and a complete end — no verdict lost" do
      Dir.mktmpdir do |dir|
        ...
        expected_turns = 0
        sum = 0.0
        described_class.arrivals(seed: seed, turns_per_hour: 12, count: 72 * 12).each do |gap|
