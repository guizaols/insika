...
RSpec.describe Insika::Safety::InputGuardrail do
  ...
  def state(message, guardrails: nil)
    ...
    Insika::TurnState.new(task: nil, profile: prof, turn: 1, message: message)
  end
  ...
  describe "moderator tier (opt-in)" do
    def moderator_factory(reply)
      lambda do |_config|
        Insika::Safety::Moderator.new(ask: ->(_p) { reply })
      end
    end
