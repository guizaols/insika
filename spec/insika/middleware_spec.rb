...
RSpec.describe Insika::MiddlewareStack do
  ...
  def logging_mw(log, tag)
    Class.new(Insika::Middleware) do
  ...
  it "base Middleware is pass-through (terminal runs, state unchanged)" do
    stack = described_class.new([Insika::Middleware.new])
    ran = false
    st = state
  ...
  it "state modification is visible to the next link and the terminal" do
    writer = Class.new(Insika::Middleware) do
  ...
  it "exception in a link propagates (stack does not rescue)" do
    boom = Class.new(Insika::Middleware) do
  ...
  it "returns the terminal's value" do
    expect(described_class.new([Insika::Middleware.new]).call(state) { |_s| :resultado }).to eq(:resultado)
  end
end
