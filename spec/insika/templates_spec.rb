...
# Insika::Templates — the template gallery. Frontmatter parsing is
# pure (tested via .send, no filesystem); the roster tests below run against
# the REAL lib/insika/templates/ tree — this IS the E3 conformance spec
# §5 asks for: the roster is linted, not trusted.
RSpec.describe Insika::Templates do
  ...
  # §5 E3 — "the roster is linted, not trusted": iterates every REAL
  ...
  describe "wave-1 roster (E3 conformance)" do
    ...
    described_class.names.each do |name|
      it "'#{name}' evaluates cleanly to schema-valid pack(s), with EVERY tool/mcp group covered by an allowlist, no secrets, only public hosts" do
        ...
        # pack(s) — the gap this same PR fixed (mcp auto-adds it;
        ...
        # declared external reference, same as E3 requires.
