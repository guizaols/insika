...
RSpec.describe Insika::Server::A2A::Client do
  # http fake: a queue of responses (JSON-RPC envelopes), records the requests.
  class FakeHttp
  ...
  describe "#send_message" do
    it "builds the message/send envelope (role user, TextPart, contextId) and returns the Task" do
    ...
    it "envelope error -> RemoteError" do
      ...
      client = described_class.new(http: http, sleeper: no_sleep)
      expect { client.send_message("u", "x") }.to raise_error(Insika::Server::A2A::RemoteError, /sem task/)
    end
  end

  describe "#call (send + poll)" do
    it "polls until completed -> { text: }" do
    ...
    it "ALWAYS does >=1 get_task (message/send without content) -> gets the text from get_task" do
      # send returns completed WITHOUT a message (like the inbound message_send); the
      # get_task carries the content.
    ...
    it "failed without a message -> error = state (avoids the '' truthy footgun)" do
      http = FakeHttp.new([envelope_task("failed"), envelope_task("failed")])
      client = described_class.new(http: http, sleeper: no_sleep)
      expect(client.call("u", "x")[:error]).to eq("failed")
    end

    it "input-required -> { text: } (the model decides)" do
    ...
    it "envelope error during the flow -> { error: } (call never raises)" do
    ...
    it "poll exhausts (poll_max) -> { error: }" do
