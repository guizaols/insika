...
RSpec.describe Insika::ModelResolver do
  ...
  def settings(overrides = {})
    Class.new do
      ...
      def get = @h
    end.new(Insika::SettingsStore::DEFAULTS.merge(overrides))
  end
  ...
  def pinned_session(model:, provider: nil)
    ...
    session_with({ Insika::ModelResolver::SESSION_SLOT => slot })
  end

  def profile(**kw) = Insika::AgentProfile.build(id: "bia", **kw)
  ...
  describe "precedence Chat > Agent > platform default" do
    ...
    it "raises a clear error when NO layer resolves a model" do
      ...
      end.to raise_error(Insika::Error, /no model resolved/)
    end
  ...
  describe "model_policy enforcement (on the RESOLVED model)" do
    it "denies a chat pin that escapes the agent fence" do
      ...
      end.to raise_error(Insika::PolicyDenied) { |e| expect(e.policy).to eq(:model_policy) }
    end
  ...
  describe "reasoning (thinking) resolution across layers" do
    def resolve_thinking(agent: nil, chat: nil, global: nil, model_params: nil, model: "m", provider: :deepseek)
      ...
      sess = chat ? session_with({ Insika::ModelResolver::SESSION_SLOT => { "thinking" => chat } }) : nil
