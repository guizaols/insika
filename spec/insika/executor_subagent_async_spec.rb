...
require "spec_helper"
require "async"
require "insika/tools/subagent" # the delivery turn (parent has subagents) wires it;
...
RSpec.describe "Insika::Executor async delegation (RFC-0010 Fase 2)" do
  ...
  let(:delegation_store) { Insika::DelegationStore.new(store: backend) }
  let(:event_stream) { SpyEventStream.new }

  let(:child_profile) { Insika::AgentProfile.build(id: "researcher", model: "gpt", base_prompt: "CHILD") }
  let(:parent_profile) do
    Insika::AgentProfile.build(id: "parent", model: "pm", base_prompt: "PARENT", subagents: ["researcher"])
  end
  ...
  describe "delivery as a NEW turn on the parent session" do
    ...
    it "delivers an ERROR note when the child turn fails" do
      executor = build_executor
      dispatch(executor, raise_child: [Insika::ProviderError, "provider down"])
  ...
  describe "boot recovery (recover_delegations)" do
    ...
    it "leaves a delegation whose child is still running (the task Recovery will resume it)" do
      executor = build_executor
      task_store.create(command: Insika::Command.build(:send_message, { agent: "researcher", message: "t" }).to_h,
