...
# Prova de correção do conversor one-off (Fase 7, Etapa D / task 7). HERMÉTICO:
...
RSpec.describe OpenclawToManifest do
  ...
  describe "o manifesto emitido" do
    ...
    # (a validação real do motor) sem levantar. secrets/env injetados fora do disco.
    let(:definitions) do
    ...
    it "cada definição valida como ToolDefinition" do
  ...
  describe "side_effect (reads não mutam)" do
  ...
  describe "param aninhado -> JSON Schema aninhado de verdade" do
    ...
    it "raiz é object com required derivado do (não-)Optional" do
      expect(params["type"]).to eq("object")
      # query_filter_pairs NÃO é Optional -> required; catalog_mode é Optional -> não.
    ...
    it "aninha object dentro do item do array, com required e additionalProperties" do
      ...
      # query obrigatório; filters Optional
  ...
  describe "defaults do manifesto (binding comum)" do
    it "secret é referência (nunca literal — R3) e fica em secret_headers" do
