...
RSpec.describe Harness::Commands::ImportTools do
  ...
  def achei_manifest(tools)
    { "version" => 1,
      "defaults" => { "base_url" => "{{env.ACHEI_URL}}", "path_template" => "/api/internal/agent_tools/{endpoint}",
                      "method" => "POST",
  ...
  it "batch upsert, hot reload (overlay+catalog) and per-tool report" do
    report = run(achei_manifest([tool("search_products"), tool("search_faq", "endpoint" => "search_faqs")]))
  ...
  it "idempotent: re-importing reconciles (updated, not created); secret preserved" do
    run(achei_manifest([tool("t")]))
    report = run(achei_manifest([tool("t", "description" => "v2")]))
  ...
  describe "isolated partial failure (R4)" do
    ...
    it "collision with a code tool becomes an isolated error (R3), does not bring down the batch" do
      report = run(achei_manifest([tool("menu"), tool("ok")]))
    ...
    it "a secret not configured in the deployment becomes an isolated per-tool error" do
      ...
      report = cmd.call(Harness::Command.build(:import_tools, achei_manifest([tool("t")]), transport: :test))
      expect(report[:created]).to be_empty
    ...
    it "a 100%-error batch does NOT reload the overlay (nothing valid)" do
      ...
      report = run(achei_manifest([tool("menu")])) # only the collision
      expect(report[:errors].size).to eq(1)
    end
  end
