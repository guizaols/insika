...
RSpec.describe "Harness::Executor + EdgeLimiter (item 33)" do
  ...
  let(:ledger) { Harness::UsageLedger.new(store: Harness::Stores::Memory.new) }
  let(:profile) do
    Harness::AgentProfile.build(id: "example-agent", model: "gpt", base_prompt: "SOUL",
                                limits: { chat_rate_limit: 1 })
  end

  def build_executor
    edge = Harness::EdgeLimiter.new(ledger: ledger)
    Harness::Executor.new(
      context_builder: FakeContextBuilder.new, policy_engine: NullPolicyEngine.new,
      middleware: Harness::MiddlewareStack.new([edge, guardrails.input_guardrail]),
