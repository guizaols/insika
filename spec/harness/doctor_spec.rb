...
RSpec.describe Harness::Doctor do
  let(:backend) { Harness::Stores::Memory.new }
  let(:config_store) { Harness::ConfigStore.new(store: backend) }
  let(:settings_store) { Harness::SettingsStore.new(config_store: config_store) }
  let(:llm_provider_store) { Harness::LLMProviderStore.new(config_store: config_store) }
  ...
  describe "#run" do
    ...
    it "ok when the backend is durable SQLite" do
      ...
      cs = Harness::ConfigStore.new(store: sqlite)
      doc = described_class.new(env: {}, settings_store: Harness::SettingsStore.new(config_store: cs),
                                llm_provider_store: Harness::LLMProviderStore.new(config_store: cs), backend: sqlite)
    ...
    it "a crashing check degrades to an error finding (never blows up run)" do
      broken = instance_double(Harness::SettingsStore)
  ...
  describe "#fix!" do
    it "runs the fixable findings and re-diagnoses to green" do
      ...
      expect(settings_store.stored_schema_version).to eq(Harness::SettingsStore::SCHEMA_VERSION)
