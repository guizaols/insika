...
RSpec.describe Harness::PendingActionStore do
  ...
  it "create devolve :pending com os campos preenchidos" do
    pa = create
  ...
  it "find recupera por id; nil se ausente" do
    pa = create
    expect(store.find(pa.id).id).to eq(pa.id)
    expect(store.find("ghost")).to be_nil
  end

  it "normaliza symbol->string na escrita (round-trip de args aninhado)" do
  ...
  it "open_for filtra por task_id e status :pending" do
    a = create(task_id: "t1")
    create(task_id: "t2")
    ...
    expect(open.map(&:id)).to eq([a.id]) # só a :pending da t1
  end

  it "resolve approved grava decisão, operador e timestamp" do
    pa = create
  ...
  it "resolve de ausente -> NotFoundError" do
    expect { store.resolve("ghost", decision: :approved) }.to raise_error(Harness::NotFoundError)
  end

  it "resolve dupla -> ValidationError (só :pending resolve)" do
    pa = create
    store.resolve(pa.id, decision: :approved, operator: "op")
    expect { store.resolve(pa.id, decision: :rejected, operator: "op") }
      .to raise_error(Harness::ValidationError, /já resolvida/)
  end

  it "decision inválida -> ValidationError" do
    pa = create
    expect { store.resolve(pa.id, decision: :maybe) }.to raise_error(Harness::ValidationError, /decision/)
  end
end
