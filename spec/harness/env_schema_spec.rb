...
RSpec.describe Harness::EnvSchema do
  describe ".validate" do
    it "a clean environment yields no findings" do
      env = { "HARNESS_DB" => "/data/h.db", "HARNESS_PORT" => "9292", "HARNESS_OTEL" => "1" }
      expect(described_class.validate(env)).to be_empty
    end

    it "flags an unknown key in the owned HARNESS_ namespace (typo catcher)" do
    ...
    it "flags a non-integer HARNESS_PORT" do
    ...
    it "accepts a valid integer and negative integers" do
      expect(described_class.validate({ "HARNESS_SUBAGENT_FANOUT_CAP" => "8" })).to be_empty
    end

    it "flags a non-boolean flag" do
      findings = described_class.validate({ "HARNESS_EGRESS_ALLOW_HTTP" => "maybe" })
      expect(findings.map(&:kind)).to eq([:invalid])
    end

    it "accepts the full boolean vocabulary case-insensitively" do
      %w[1 0 true FALSE yes No on OFF].each do |v|
        expect(described_class.validate({ "HARNESS_OTEL" => v })).to be_empty
      end
    end

    it "flags a malformed URL" do
      findings = described_class.validate({ "HARNESS_PUBLIC_URL" => "not-a-url" })
      expect(findings.map(&:kind)).to eq([:invalid])
    end

    it "accepts an http(s) URL" do
      expect(described_class.validate({ "HARNESS_PUBLIC_URL" => "https://x.example/y" })).to be_empty
    end

    it "a blank value on an OPTIONAL key is not a finding" do
      expect(described_class.validate({ "HARNESS_PORT" => "" })).to be_empty
    end
  ...
  describe ".enforce!" do
    it "WARNS on findings and returns them without raising (last-known-good default)" do
      warned = []
      findings = described_class.enforce!({ "HARNESS_BOGUS" => "1" }, strict: false, warn: ->(m) { warned << m })
      expect(findings.map(&:kind)).to eq([:unknown])
      expect(warned).to include(a_string_matching(/HARNESS_BOGUS/))
    end

    it "RAISES ConfigError with error findings when strict" do
      expect { described_class.enforce!({ "HARNESS_BOGUS" => "1" }, strict: true, warn: ->(_) {}) }
        .to raise_error(Harness::ConfigError) { |e| expect(e.findings.map(&:key)).to eq(["HARNESS_BOGUS"]) }
    end
    ...
    it "derives strictness from HARNESS_CONFIG_STRICT when strict: nil" do
      expect { described_class.enforce!({ "HARNESS_CONFIG_STRICT" => "1", "HARNESS_BOGUS" => "1" }, warn: ->(_) {}) }
        .to raise_error(Harness::ConfigError)
      expect { described_class.enforce!({ "HARNESS_BOGUS" => "1" }, warn: ->(_) {}) }.not_to raise_error
    end
  end
