...
RSpec.describe Harness::Safety::Factory do
  def state(guardrails)
    prof = Harness::AgentProfile.build(id: "x", guardrails: guardrails)
    Harness::TurnState.new(task: nil, profile: prof, turn: 1, message: "oi")
  end
  ...
  describe "#content_filter_factory" do
    it "builds a fresh OutputFilter when the agent has output guardrails on" do
      ...
      expect(one).to be_a(Harness::Safety::OutputFilter)
  ...
  describe "moderator model resolution" do
    let(:settings) do
      Harness::SettingsStore.new(config_store: Harness::ConfigStore.new(store: Harness::Stores::Memory.new))
    end

    it "resolves the per-agent moderator ref into a Moderator" do
      factory = described_class.new(settings_store: settings)
      config = Harness::Safety::Config.from_hash("moderator" => "deepseek/deepseek-chat")
      expect(factory.send(:moderator_for, config)).to be_a(Harness::Safety::Moderator)
    end

    it "'on' falls back to the platform utility_model" do
      settings.update("utility_model" => "deepseek/deepseek-chat")
      factory = described_class.new(settings_store: settings)
      config = Harness::Safety::Config.from_hash("moderator" => "on")
      expect(factory.send(:moderator_for, config)).to be_a(Harness::Safety::Moderator)
    end
    ...
    it "no moderator opt-in -> nil" do
      factory = described_class.new(settings_store: settings)
      expect(factory.send(:moderator_for, Harness::Safety::Config.from_hash({}))).to be_nil
    end
  end
end
