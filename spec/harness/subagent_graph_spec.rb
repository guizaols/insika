...
RSpec.describe Harness::SubagentGraph do
  describe ".validate! over a {id => [children]} map" do
    ...
    it "raises SubagentCycleError on a direct cycle, exposing the path" do
      expect { described_class.validate!({ "a" => ["b"], "b" => ["a"] }) }
        .to raise_error(Harness::SubagentCycleError) { |e| expect(e.cycle).to eq(%w[a b a]) }
    end
    ...
    it "raises SubagentDepthExceeded when the longest chain passes the cap" do
      ...
      expect { described_class.validate!(chain, cap: 5) }
        .to raise_error(Harness::SubagentDepthExceeded) { |e| expect(e.depth).to eq(6) }
    end
    ...
    it "the errors are ValidationErrors (a bad graph fails the authoring Command cleanly)" do
      expect(Harness::SubagentCycleError.ancestors).to include(Harness::ValidationError)
      expect(Harness::SubagentDepthExceeded.ancestors).to include(Harness::ValidationError)
    end
  end

  describe ".validate! over an array of AgentProfiles" do
    it "reads #id and #subagents and validates the whole set" do
      parent = Harness::AgentProfile.build(id: "parent", subagents: ["child"])
      child = Harness::AgentProfile.build(id: "child")
      expect { described_class.validate!([parent, child]) }.not_to raise_error
    end

    it "catches a cycle across two profiles" do
      a = Harness::AgentProfile.build(id: "a", subagents: ["b"])
      b = Harness::AgentProfile.build(id: "b", subagents: ["a"])
      expect { described_class.validate!([a, b]) }.to raise_error(Harness::SubagentCycleError)
    end
  end
