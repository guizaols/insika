...
RSpec.describe "Harness::ChatBuilder subagent wiring (RFC-0010)" do
  ...
  def builder(subagent_runner:)
    Harness::ChatBuilder.new(tool_registry: inert, skill_catalog: skill_catalog,
                             checkpoint_store: inert, event_stream: inert,
                             hooks: Harness::Hooks.new, subagent_runner: subagent_runner)
  end

  def state(subagents:)
    profile = Harness::AgentProfile.build(id: "parent", model: "m", subagents: subagents)
