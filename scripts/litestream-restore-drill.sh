...
#   1. seeds a marker row into a fresh insika.db (host sqlite3, WAL);
#   2. boots the image with Litestream ENABLED → replicates to a local replica;
#   3. wipes the data volume (simulating a lost box);
#   4. boots a SECOND container → entrypoint restores insika.db from the replica;
#   5. asserts the marker row survived AND /up is green.
#
# Requires: docker, sqlite3, curl. Usage:
#   scripts/litestream-restore-drill.sh [image-tag]   (default: insika:g1-drill)
set -euo pipefail

IMAGE="${1:-insika:g1-drill}"
...
REPLICA_URL="file:///replica/insika"
MARKER="drill-$(date +%s)"
...
run_container() {
  ...
  docker run -d --name "insika-drill-$1" \
    -p 0:9292 \
    -v "${DATA}:/data" -v "${REPLICA}:/replica" \
    -e "INSIKA_DB=/data/insika.db" \
    -e "ADMIN_TOKEN=drill" \
...
rm -f "${DATA}"/insika.db "${DATA}"/insika.db-shm "${DATA}"/insika.db-wal
test ! -f "${DATA}/insika.db" || { echo "  FAIL: db not wiped" >&2; exit 1; }
echo "  ${DATA}/insika.db removed"
...
test -f "${DATA}/insika.db" || { echo "  FAIL: db not restored" >&2; exit 1; }
GOT="$(sqlite3 "${DATA}/insika.db" "SELECT tag FROM drill_marker LIMIT 1;")"
if [ "${GOT}" = "${MARKER}" ]; then
