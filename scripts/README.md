...
- **`loadtest_session.rb`** — session-level benchmark: a full multi-message
  ...
  (burst, verdicts counted) mode. Two surfaces: `engine` (direct to
  `/v1/responses`) or `web` (the production path through a consumer's widget
  API — needs `--widget-id`). Local or Railway via `INSIKA_URL`.
