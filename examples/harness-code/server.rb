...
# Launches the harness-code prototype server: a single-process Async::HTTP
...
# POST /v1/commands/approve_action, which the CLI (bin/harness-code) drives.
#
# Usage:
#   HARNESS_CODE_ROOT=/path/to/project \
#   DEEPSEEK_API_KEY=... \
#   ruby examples/harness-code/server.rb           # binds http://localhost:9292
#
# Env:
...
puts "  bind      : #{BIND}"
puts "  token     : #{HarnessCodeApp::GATEWAY_TOKEN}"
...
puts "  CLI       : HARNESS_CODE_URL=#{BIND} HARNESS_CODE_TOKEN=#{HarnessCodeApp::GATEWAY_TOKEN} " \
